...
test('a configuração declarativa possui vinte conteúdos e vinte e seis artes', async () => {
  const contents = await listContents();
  assert.equal(contents.length, 20);
  assert.equal(contents.reduce((total, content) => total + content.artworks, 0), 26);
});

test('conteúdos publicados permanecem na pasta de arquivo', async () => {
  const contents = await listContents();
  const published = contents.filter(({ slug }) => /^(0[4-9]|1[0-9]|20)-/.test(slug));
...
test('conteúdos sem diretório explícito são gerados na pasta de novos', async () => {
  const contents = await listContents();
  const newContents = contents.filter(({ slug }) => /^(21|22|23)-/.test(slug));
